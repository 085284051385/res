...
const starts = async (dha = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('OCHOBOTZ ', {
		font: 'Standard',
	...
	dha.browserDescription = ["RIDHO ASLI", "Chrome", "3.0.0"];
	...
	dha.on('connecting', () => {
		console.log(color('[ RIDHO OFFC ]', 'yellow'), color('PROSES NYAMBUNG...'));
	})
const spinner = { 
  "interval": 120,
  "frames": [
	...
	//connect
	dha.on('open', () => {
		console.log(color('[ RIDHO OFFC ]', 'yellow'), color('BOT SUDAH AKTIF  SELAMAT MENGGUNAKAN'));
	})
